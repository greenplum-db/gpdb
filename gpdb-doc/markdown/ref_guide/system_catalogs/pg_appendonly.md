...
|`compresslevel`|smallint| |The compression level, with compression ratio increasing from 1 to 9. When `quicklz`1 is specified for compresstype, valid values are 1 or 3. With `zlib` specified, valid values are 1-9.
...
|`compresstype`|text| |Type of compression used to compress append-optimized tables. Valid values are:-   `zlib` \(gzip compression\)
-   `quicklz`1
