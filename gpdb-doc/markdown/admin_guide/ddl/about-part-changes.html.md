...
VMware Greenplum 7 retains most aspects of the partitioning syntax of prior versions of Greenplum, now referred to as the *uniform* partitioning syntax. Version 7 also introduces support for the PostgreSQL declarative partitioning syntax, the *flexible* syntax.
...
- Support for PostgreSQL declarative partitioning syntax, which introduces these new features *for the flexible syntax only*:
...
The following uniform partitioning syntax and behaviors *have not* changed in Greenplum 7:

- Greenplum 7 supports the uniform partitioning syntax of Greenplum 6 except where called out in the [What Has Changed?](#changed) section.
...
The following items describe the Greenplum 7 *changes* to uniform partitioning syntax and behaviors compared to Greenplum 6:

- New internal data structures and catalogs are used to represent partitioned tables. 
    > **Important** Greenplum 7 represents and operates on partitioned tables using the new internal data structures and catalog, regardless of the partitioning syntax used to create the table. Operations that you invoke using the uniform partitioning syntax are internally mapped to the new data structures, and any output generated by Greenplum, such as DDL, is displayed in that format.
...
- Partition-specific information displayed in `psql` `\d+` output has changed:
    ...
    - Range partition boundaries defined with `START`/`END` are displayed using the (flexible syntax) `FOR VALUES [ FROM/TO | IN | WITH ]` keywords.
    - Example:
...
- The `EXCHANGE PARTITION` command has changed:
    ...
    - This command is now internally implemented with the `DETACH PARTITION` and `ATTACH PARTITION` commands introduced with the flexible syntax.
...
- Greenplum 7 dumps and restores the DDL of partitioned tables defined with uniform syntax using the new catalog structures.
