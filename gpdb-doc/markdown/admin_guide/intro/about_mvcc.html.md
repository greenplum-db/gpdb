...
In Greenplum Database, you can request any of the four transaction isolation levels, but only two distinct isolation levels are implemented internally. The Greenplum Database `READ UNCOMMITTED` level behaves like `READ COMMITTED`, and the `SERIALIZABLE` level falls back to `REPEATABLE READ`.
