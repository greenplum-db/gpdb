...
def impl(context, table, dbname):
    ...
    for i in range(len(post_distribution_dist_policy)):
        if(post_distribution_dist_policy[i] != pre_redistribution_dist_policy[i]):
            raise Exception("Redistributed policy does not match pre-redistribution policy. Actual: %d, Expected: %d" % (post_distribution_dist_policy[i], pre_redistribution_dist_policy[i]))
...
def _get_dist_policy_per_partition(table, dbname):
    with closing(dbconn.connect(dbconn.DbURL(dbname=dbname), unsetSearchPath=False)) as conn:
        query = "SELECT pg_get_table_distributedby('%s'::regclass) AS distribution_policy;" % table
        cursor = dbconn.query(conn, query)
        rows = cursor.fetchall()
    return [row[0] for row in rows] # indices are the gp segment id's, so no need to store them explicitly
