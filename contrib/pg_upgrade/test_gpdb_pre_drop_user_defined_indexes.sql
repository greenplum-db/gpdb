...
BEGIN
    FOR user_defined_indexes IN select *, pg_class.relname as constraint_table_name from get_user_defined_indexes() 
    ...
    LOOP
        execute 'alter table ' || quote_ident(user_defined_indexes.namespace_name) || '.' || quote_ident(user_defined_indexes.constraint_table_name) || ' drop constraint if exists ' || 
