...
main(int argc, char *argv[])
{
	...
	{
		fprintf(stderr, _("%s: options -g/--globals-only and --roles-only cannot be used together\n"),
				progname);
		fprintf(stderr, _("Try \"%s --help\" for more information.\n"),
				progname);
		exit_nicely(1);
	...
	{
		fprintf(stderr, _("%s: options --roles-only and -t/--tablespaces-only cannot be used together\n"),
				progname);
		fprintf(stderr, _("Try \"%s --help\" for more information.\n"),
				progname);
		exit_nicely(1);
...
dumpCreateDB(PGconn *conn)
{
	...
	for (i = 0; i < PQntuples(res); i++)
	{
		...
		if (!skip_acls &&
			!buildACLCommands(fdbname, NULL, NULL, "DATABASE", dbacl, rdbacl, dbowner,
							  "", server_version, buf))
		{
...
dumpDbRoleConfig(PGconn *conn)
{
	...
	printfPQExpBuffer(buf, "SELECT rolname, datname, unnest(setconfig) "
					  ...
					  "WHERE setrole = u.oid AND setdatabase = pg_database.oid ORDER BY 1,2,3", role_catalog);
	res = executeQuery(conn, buf->data);
