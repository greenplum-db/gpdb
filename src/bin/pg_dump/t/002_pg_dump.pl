...
my %tests = (
	...
	'ALTER TABLE test_third_table OWNER TO' => {
		regexp => qr/^ALTER TABLE dump_test.test_third_table OWNER TO .*;/m,
		like => {
			binary_upgrade => 1,
			clean => 1,
			clean_if_exists => 1,
...
#	},
	...
	'COPY test_third_table' => {
		...
		create_sql =>
		  ...
		  . 'SELECT generate_series FROM generate_series(1,9);',
		regexp => qr/^
			\QCOPY dump_test.test_third_table (col1) FROM stdin;\E
			\n(?:\d\n){9}\\\.\n
			/xm,
		like => {
			clean => 1,
	...
	'INSERT INTO test_third_table' => {
		regexp => qr/^
			(?:INSERT\ INTO\ dump_test.test_third_table\ \(col1\)
			   \ VALUES\ \(\d\);\n){9}/xm,
		like => {
			column_inserts => 1,
		},
	...
	'CREATE MATERIALIZED VIEW matview' => {
		...
		regexp => qr/^
			...
			\n\s+\QSELECT dump_test.test_table.col1\E
	...
	'GRANT SELECT ON TABLE test_table' => {
		...
		create_sql => 'GRANT SELECT ON TABLE dump_test.test_table
						   TO dump_test;',
		regexp => qr/^GRANT SELECT ON TABLE dump_test.test_table TO dump_test;/m,
		like => {
			binary_upgrade => 1,
			clean => 1,
			clean_if_exists => 1,
	...
	'GRANT SELECT ON TABLE test_third_table' => {
		...
		create_sql => 'GRANT SELECT ON
						   ...
						   TO dump_test;',
		regexp => qr/^GRANT SELECT ON TABLE dump_test_second_schema.test_third_table TO dump_test;/m,
		like => {
			binary_upgrade => 1,
			clean => 1,
			clean_if_exists => 1,
	...
	'GRANT ALL ON SEQUENCE test_third_table_col1_seq' => {
		...
		create_sql => 'GRANT ALL ON SEQUENCE
						   ...
						   TO dump_test;',
		regexp => qr/^
			\QGRANT ALL ON SEQUENCE dump_test_second_schema.test_third_table_col1_seq TO dump_test;\E
			/xm,
		like => {
			binary_upgrade => 1,
			clean => 1,
	...
	'GRANT INSERT(col1) ON TABLE test_second_table' => {
		...
		create_sql => 'GRANT INSERT (col1) ON TABLE dump_test.test_second_table
						   TO dump_test;',
		regexp => qr/^
			\QGRANT INSERT(col1) ON TABLE dump_test.test_second_table TO dump_test;\E
			/xm,
		like => {
			binary_upgrade => 1,
			clean => 1,
	...
	'GRANT SELECT (proname ...) ON TABLE pg_proc TO public' => {
		...
		\QGRANT SELECT(proacl) ON TABLE pg_proc TO PUBLIC;\E\n.*
		\QGRANT SELECT(prodataaccess) ON TABLE pg_proc TO PUBLIC;\E\n.*
		\QGRANT SELECT(proexeclocation) ON TABLE pg_proc TO PUBLIC;\E/xms,
		like => {
			binary_upgrade => 1,
			clean => 1,
			clean_if_exists => 1,
