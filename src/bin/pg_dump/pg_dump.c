...
bool gp_partitioning_available = false;
						   ...
						   bool strict_names)
{
	...
	for (cell = patterns->head; cell; cell = cell->next)
	{
		...
		if (strict_names && PQntuples(res) == 0)
			exit_horribly(NULL, "no matching tables were found for pattern \"%s\"", cell->val);
...
dumpBlob(Archive *fout, BlobInfo *binfo)
{
	...
	if (binfo->blobacl && (binfo->dobj.dump & DUMP_COMPONENT_ACL))
		dumpACL(fout, binfo->dobj.catId, binfo->dobj.dumpId, NULL, "LARGE OBJECT",
				binfo->dobj.name, NULL,
				binfo->rolname, binfo->blobacl, binfo->rblobacl,
...
getTableAttrs(Archive *fout, TableInfo *tblinfo, int numTables)
{
	...
	for (i = 0; i < numTables; i++)
	{
		...
		for (j = 0; j < ntups; j++)
		{
			 ...
			 * extra check later.
			 */
			if (dopt->binary_upgrade && tbinfo->parparent && tbinfo->attisdropped[j])
		 ...
		 * table check.
		 */
		if (dopt->binary_upgrade && rootPartHasDroppedAttr)
		{
...
#define VALIDFN_IDX 2
	...
	/* Skip if not to be dumped */
	if (!ptcinfo->dobj.dump || dopt->dataOnly)
		return;
...
dumpForeignServer(Archive *fout, ForeignServerInfo *srvinfo)
{
	...
	if (srvinfo->dobj.dump & DUMP_COMPONENT_ACL)
		dumpACL(fout, srvinfo->dobj.catId, srvinfo->dobj.dumpId,
				...
				srvinfo->dobj.namespace->dobj.name, srvinfo->rolname,
		...
		const char *initacls, const char *initracls)
{
	...
	if (!buildACLCommands(name, subname, nspname, type, acls, racls, owner,
						  "", fout->remoteVersion, sql))
		exit_horribly(NULL,
		...
		ArchiveEntry(fout, tbinfo->dobj.catId, tbinfo->dobj.dumpId,
				...
				(tbinfo->relkind == RELKIND_VIEW) ? NULL : tbinfo->reltablespace,
					...
					(strcmp(reltypename, "TABLE") == 0 ||
						...
						reltypename, tbinfo->postponed_def ?
