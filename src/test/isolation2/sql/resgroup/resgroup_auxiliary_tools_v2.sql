...
0: CREATE OR REPLACE FUNCTION check_cgroup_configuration() RETURNS BOOL AS $$
    ...
    shares = get_cgroup_prop('gpdb.slice/cpu.weight')
    ...
    def check_group_shares(name):
        ...
        oid = int(plpy.execute('''
                ...
                '''.format(name))[0]['oid'])
        sub_shares = get_cgroup_prop('gpdb.slice/{}/cpu.weight'.format(oid))
...
0: CREATE FUNCTION check_cpuset(grp TEXT, cpuset TEXT) RETURNS BOOL AS $$
    ...
    conf = cpuset
    if conf == '':
        fd = open("/sys/fs/cgroup/gpdb.slice/cpuset.cpus")
        line = fd.readline()
        fd.close()
        conf = line.strip('\n')
...
0: CREATE OR REPLACE FUNCTION create_allcores_group(grp TEXT) RETURNS BOOL AS $$
    ...
    file = "/sys/fs/cgroup/gpdb.slice/cpuset.cpus"
    fd = open(file)
    line = fd.readline()
    fd.close()
    line = line.strip('\n')
    ...
    file = "/sys/fs/cgroup/gpdb.slice/1/cpuset.cpus"
    fd = open(file)
    line = fd.readline()
    fd.close()
    line = line.strip('\n')
...
0: CREATE OR REPLACE FUNCTION check_cpuset_rules() RETURNS BOOL AS $$
    ...
    def get_cgroup_cpuset(group):
        ...
        if group == '0':
            ...
            file = "/sys/fs/cgroup/gpdb.slice/" + group + "/cpuset.cpus"
        fd = open(file)
        line = fd.readline()
        fd.close()
...
0: CREATE OR REPLACE FUNCTION is_session_in_group(pid integer, groupname text) RETURNS BOOL AS $$
    ...
    def get_result(host):
        ...
        path = "/sys/fs/cgroup/gpdb.slice/{}/cgroup.procs".format(groupid)
