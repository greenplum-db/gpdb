...
select pg_catalog.catDML(
'INSERT INTO pg_catalog.pg_depend SELECT 0,0,0, tableoid,oid,0, ''p'' FROM pg_catalog.pg_operator
        where (tableoid,oid) not in (Select refclassid, refobjid from pg_catalog.pg_depend where deptype = ''p'')
          and oid < 10000'
);
select pg_catalog.catDML(
'INSERT INTO pg_catalog.pg_depend SELECT 0,0,0, tableoid,oid,0, ''p'' FROM pg_catalog.pg_cast
        where (tableoid,oid) not in (Select refclassid, refobjid from pg_catalog.pg_depend where deptype = ''p'')
          and oid < 10000'
);
