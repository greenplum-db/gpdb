...
create or replace function pg_catalog.addProcPIN(procid oid) returns int as $$
begin
  INSERT INTO pg_catalog.pg_depend values(0,0,0, 1255, procid, 0, 'p');
  PERFORM pg_catalog.catDML('INSERT INTO pg_catalog.pg_depend values(0,0,0, 1255, '||procid||', 0, ''p'')') from gp_dist_random('gp_id');
  return 1;
end;
$$ language 'plpgsql';
...
INSERT INTO pg_catalog.pg_depend SELECT 0,0,0, tableoid,oid,0, 'p' FROM pg_catalog.pg_proc
        where (tableoid,oid) not in (Select refclassid, refobjid from pg_catalog.pg_depend)
          and pronamespace = 11
          and oid < 16384;
...
drop function pg_catalog.addProcPIN(procid oid);
abort;
