...
/* TIDYCAT_BEGINDEF
   ...
   with (relid=3079, toast_oid=5510, toast_index=5511)
   ( 
    extname           name,
    extowner          oid,
    extnamespace      oid,
   ...
   alter table pg_extension add fk extowner on pg_authid(oid);
   alter table pg_extension add fk extnamespace on pg_namespace(oid);
