...
//---------------------------------------------------------------------------
void
CTranslatorDXLToPlStmt::MapLocationsFile(OID oidRel, char **rgszSegFileMap,
										 CdbComponentDatabases *pcdbCompDB)
{
	...
	ForEach(plcLocation, extentry->urilocations)
	{
		...
		{
			CdbComponentDatabaseInfo *pcdbCompDBInfo =
				&pcdbCompDB->segment_db_info[i];
			INT iSegInd = pcdbCompDBInfo->segindex;
			if (SEGMENT_IS_ACTIVE_PRIMARY(pcdbCompDBInfo))
			{
				if (URI_FILE == pUri->protocol &&
					0 != gpdb::StrCmpIgnoreCase(pUri->hostname,
												pcdbCompDBInfo->hostname) &&
					0 != gpdb::StrCmpIgnoreCase(pUri->hostname,
												pcdbCompDBInfo->address))
				{
					continue;
				}
		...
		{
			GPOS_RAISE(
				gpdxl::ExmaDXL, gpdxl::ExmiDXL2PlStmtExternalScanError,
				GPOS_WSZ_LIT(
					"Could not assign a segment database for external file"));
		}
	}
}
...
//---------------------------------------------------------------------------
void
CTranslatorDXLToPlStmt::MapLocationsFdist(OID oidRel, char **rgszSegFileMap,
										  CdbComponentDatabases *pcdbCompDB,
										  Uri *pUri,
										  const ULONG ulTotalPrimaries)
{
	...
	while (!fDone)
	{
		...
		ForEach(plcLocation, extentry->urilocations)
		{
			Value *pvLocation = (Value *) lfirst(plcLocation);
			CHAR *szUri = pvLocation->val.str;
			plModifiedLocations = gpdb::LAppend(plModifiedLocations,
												gpdb::MakeStringValue(szUri));
			ulModifiedLocations++;
...
//---------------------------------------------------------------------------
void
CTranslatorDXLToPlStmt::MapLocationsExecute(OID oidRel, char **rgszSegFileMap,
											CdbComponentDatabases *pcdbCompDB,
											const ULONG ulTotalPrimaries)
{
	ExtTableEntry *extentry = gpdb::GetExternalTableEntry(oidRel);
	...
	{
		MapLocationsExecuteAllSegments(szPrefixedCommand, rgszSegFileMap,
									   pcdbCompDB);
	}
	else if (0 == gpos::clib::Strcmp(szOnClause, "PER_HOST"))
	{
		MapLocationsExecutePerHost(szPrefixedCommand, rgszSegFileMap,
								   pcdbCompDB);
	}
	else if (0 == gpos::clib::Strncmp(szOnClause,
									  "HOST:", gpos::clib::Strlen("HOST:")))
	{
		CHAR *szHostName = szOnClause + gpos::clib::Strlen("HOST:");
		MapLocationsExecuteOneHost(szHostName, szPrefixedCommand,
								   rgszSegFileMap, pcdbCompDB);
	}
	else if (0 == gpos::clib::Strncmp(szOnClause, "SEGMENT_ID:",
									  gpos::clib::Strlen("SEGMENT_ID:")))
	{
		CHAR *pcEnd = NULL;
		INT iTargetSegInd = (INT) gpos::clib::Strtol(
			szOnClause + gpos::clib::Strlen("SEGMENT_ID:"), &pcEnd, 10);
		MapLocationsExecuteOneSegment(iTargetSegInd, szPrefixedCommand,
									  rgszSegFileMap, pcdbCompDB);
	}
	else if (0 == gpos::clib::Strncmp(szOnClause, "TOTAL_SEGS:",
									  gpos::clib::Strlen("TOTAL_SEGS:")))
	{
		// total n segments selected randomly
		CHAR *pcEnd = NULL;
		ULONG ulSegsToUse = gpos::clib::Strtol(
			szOnClause + gpos::clib::Strlen("TOTAL_SEGS:"), &pcEnd, 10);

		MapLocationsExecuteRandomSegments(ulSegsToUse, ulTotalPrimaries,
										  szPrefixedCommand, rgszSegFileMap,
										  pcdbCompDB);
	}
...
void
CTranslatorDXLToPlStmt::MapLocationsExecuteAllSegments(
	CHAR *szPrefixedCommand, char **rgszSegFileMap,
	CdbComponentDatabases *pcdbCompDB)
{
	...
	for (int i = 0; i < pcdbCompDB->total_segment_dbs; i++)
	{
		...
		if (SEGMENT_IS_ACTIVE_PRIMARY(pcdbCompDBInfo))
		{
			...
			ForEach(plc, plVisitedHosts)
			{
				...
				if (0 == gpdb::StrCmpIgnoreCase(szHostName,
												pcdbCompDBInfo->hostname))
				{
					fHostTaken = true;
					break;
...
void
CTranslatorDXLToPlStmt::MapLocationsExecuteOneHost(
	CHAR *szHostName, CHAR *szPrefixedCommand, char **rgszSegFileMap,
	CdbComponentDatabases *pcdbCompDB)
{
	BOOL fMatchFound = false;
	for (int i = 0; i < pcdbCompDB->total_segment_dbs; i++)
	...
	if (!fMatchFound)
	{
		GPOS_RAISE(
			gpdxl::ExmaDXL, gpdxl::ExmiDXL2PlStmtExternalScanError,
			GPOS_WSZ_LIT(
				"Could not assign a segment database for given command. No valid primary segment was found in the requested host name."));
	}
}
...
void
CTranslatorDXLToPlStmt::MapLocationsExecuteOneSegment(
	INT iTargetSegInd, CHAR *szPrefixedCommand, char **rgszSegFileMap,
	CdbComponentDatabases *pcdbCompDB)
{
	BOOL fMatchFound = false;
	for (int i = 0; i < pcdbCompDB->total_segment_dbs; i++)
	...
	if (!fMatchFound)
	{
		GPOS_RAISE(
			gpdxl::ExmaDXL, gpdxl::ExmiDXL2PlStmtExternalScanError,
			GPOS_WSZ_LIT(
				"Could not assign a segment database for given command. The requested segment id is not a valid primary segment."));
	}
}
...
void
CTranslatorDXLToPlStmt::MapLocationsExecuteRandomSegments(
	ULONG ulSegments, const ULONG ulTotalPrimaries, CHAR *szPrefixedCommand,
	char **rgszSegFileMap, CdbComponentDatabases *pcdbCompDB)
{
...
CTranslatorDXLToPlStmt::PlExternalScanUriList(OID oidRel)
{
	...
	rgszSegFileMap =
		(char **) gpdb::GPDBAlloc(ulTotalPrimaries * sizeof(char *));
	...
	{
		szFirstUri =
			((Value *) gpdb::ListNth(extentry->urilocations, 0))->val.str;
		pUri = gpdb::ParseExternTableUri(szFirstUri);
	}

	if (fUsingLocation &&
		(URI_FILE == pUri->protocol || URI_HTTP == pUri->protocol))
	{
		MapLocationsFile(oidRel, rgszSegFileMap, pcdbCompDB);
	}
	else if (fUsingLocation &&
			 (URI_GPFDIST == pUri->protocol || URI_GPFDISTS == pUri->protocol ||
			  URI_CUSTOM == pUri->protocol))
	{
		MapLocationsFdist(oidRel, rgszSegFileMap, pcdbCompDB, pUri,
						  ulTotalPrimaries);
	}
	else if (fUsingExecute)
	{
		MapLocationsExecute(oidRel, rgszSegFileMap, pcdbCompDB,
							ulTotalPrimaries);
	}
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	if (IMDRelation::ErelstorageExternal == md_rel->RetrieveRelStorageType())
	{
		...
		ext_scan->isMasterOnly = (IMDRelation::EreldistrMasterOnly ==
								  md_rel_ext->GetRelDistribution());
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	return TranslateDXLIndexScan(index_scan_dxlnode, physical_idx_scan_dxlop,
								 output_context, false /*is_index_only_scan*/,
								 ctxt_translation_prev_siblings);
}
...
CTranslatorDXLToPlStmt::TranslateDXLIndexScan(
	...
	CDXLPhysicalIndexScan *physical_idx_scan_dxlop,
	CDXLTranslateContext *output_context, BOOL is_index_only_scan,
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	// translation context for column mappings in the base relation
	CDXLTranslateContextBaseTable base_table_context(m_mp);
	...
	RangeTblEntry *rte = TranslateDXLTblDescrToRangeTblEntry(
		physical_idx_scan_dxlop->GetDXLTableDescr(), index_descr_dxl, index,
		&base_table_context);
	GPOS_ASSERT(NULL != rte);
	rte->requiredPerms |= ACL_SELECT;
	m_dxl_to_plstmt_context->AddRTE(rte);
...
CTranslatorDXLToPlStmt::TranslateIndexConditions(
	...
	BOOL is_index_only_scan, BOOL is_bitmap_index_probe, const IMDIndex *index,
	...
	List **index_subtype_list)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		if (!is_index_only_scan)
		{
			...
			SetIndexVarAttnoWalker((Node *) index_cond_expr,
								   &index_varattno_ctxt);
		}
	...
	for (ULONG ul = 0; ul < length; ul++)
	{
		...
		*index_strategy_list = gpdb::LAppendInt(
			*index_strategy_list, index_qual_info->m_strategy_num);
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	ForEach(lc_target_entry, target_list)
	{
		...
		func_scan->funccoltypes =
			gpdb::LAppendOid(func_scan->funccoltypes, oid_type);
		func_scan->funccoltypmods =
			gpdb::LAppendInt(func_scan->funccoltypmods, typ_mod);
	}
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
		...
		motion_dxlop->GetDXLOperator() == EdxlopPhysicalMotionRandom)
	{
		...
		if (EdxlopPhysicalMotionRedistribute == motion_dxlop->GetDXLOperator())
		{
			...
			TranslateHashExprList(hash_expr_list_dxlnode, &child_context,
								  &hash_expr_list, &hash_expr_types_list,
								  output_context);
		}
		GPOS_ASSERT(gpdb::ListLength(hash_expr_list) ==
					gpdb::ListLength(hash_expr_types_list));
	...
	switch (motion_dxlop->GetDXLOperator())
	{
		case EdxlopPhysicalMotionGather:
		{
			...
			// get segment id
			INT segid =
				CDXLPhysicalGatherMotion::Cast(motion_dxlop)->IOutputSegIdx();
		...
		case EdxlopPhysicalMotionRandom:
		{
			...
			GPOS_ASSERT(NULL != output_segids_array &&
						0 < output_segids_array->Size());
			ULONG segid_count = output_segids_array->Size();
			motion->outputSegIdx =
				(INT *) gpdb::GPDBAlloc(segid_count * sizeof(INT));
	...
	if (EdxlopPhysicalMotionRedistribute == motion_dxlop->GetDXLOperator())
	{
		...
		for (ULONG ul = 0; ul < length; ul++)
		{
			...
			else
			{
				...
				plan->targetlist =
					gpdb::LAppend(plan->targetlist, target_entry);
	...
	if (targetlist_modified)
	{
		...
		ForEach(lc, child_result->targetlist)
		{
			...
			Var *var = gpdb::MakeVar(
				OUTER, te->resno, gpdb::ExprType((Node *) te->expr),
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	if (window->numPartCols > 0)
	{
		window->partColIdx = (AttrNumber *) gpdb::GPDBAlloc(
			window->numPartCols * sizeof(AttrNumber));
		window->partOperators =
			(Oid *) gpdb::GPDBAlloc(window->numPartCols * sizeof(Oid));
	}
	...
	for (ULONG ul = 0; ul < size; ul++)
	{
		...
		const CDXLWindowKey *window_key_dxl_op =
			window_dxlop->GetDXLWindowKeyAt(ul);
		const CDXLNode *sort_col_list_dxlnode =
			window_key_dxl_op->GetSortColListDXL();
		...
		TranslateSortCols(sort_col_list_dxlnode, &child_context,
						  windowkey->sortColIdx, windowkey->sortOperators,
						  windowkey->nullsFirst);
		...
		{
			windowkey->frame =
				Pwindowframe(window_key_dxl_op->GetWindowFrame(),
							 &child_context, output_context, plan);
		}
									 ...
									 Plan *pplan)
{
	...
	window_frame->exclude = CTranslatorUtils::Windowexclusion(
		pdxlwf->ParseFrameExclusionStrategy());
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	TranslateSortCols(sort_col_list_dxl, &child_context, sort->sortColIdx,
					  sort->sortOperators, sort->nullsFirst);
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	CDXLPhysicalAppend *pdxlopAppend =
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	if (!IsA(child_plan, Material) && !IsA(child_plan, Sort))
	{
		...
		ForEach(lc_target_entry, plan->targetlist)
		{
			...
			Var *var = (Var *) expr;
			Var *var_new = gpdb::MakeVar(OUTER, var->varattno, var->vartype,
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	for (ULONG ul = 0; ul < num_of_proj_list_elem; ul++)
	{
		...
		Var *var =
			gpdb::MakeVar(OUTER, (AttrNumber)(ul + 1), oid_type,
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	RangeTblEntry *rte = TranslateDXLTblDescrToRangeTblEntry(
		dyn_tbl_scan_dxlop->GetDXLTableDescr(), NULL /*index_descr_dxl*/, index,
		&base_table_context);
	GPOS_ASSERT(NULL != rte);
	rte->requiredPerms |= ACL_SELECT;
	...
	dyn_tbl_scan->partIndexPrintable =
	...
	CDXLTranslationContextArray *ctxt_translation_prev_siblings)
{
	...
	RangeTblEntry *rte = TranslateDXLTblDescrToRangeTblEntry(
		dyn_index_scan_dxlop->GetDXLTableDescr(), NULL /*index_descr_dxl*/,
		index, &base_table_context);
	GPOS_ASSERT(NULL != rte);
	rte->requiredPerms |= ACL_SELECT;
	m_dxl_to_plstmt_context->AddRTE(rte);
	...
	dyn_idx_scan->scan.partIndexPrintable =
	...
	dyn_idx_scan->indexorderdir = CTranslatorUtils::GetScanDirection(
...
CTranslatorDXLToPlStmt::TranslateDXLTblDescrToRangeTblEntry(
	...
	const CDXLIndexDescr
		*index_descr_dxl,  // should be NULL unless we have an index-only scan
	Index index, CDXLTranslateContextBaseTable *base_table_context)
{
	...
	CDXLTranslateContext *output_context)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		Var *var =
			gpdb::MakeVar(OUTER, te_child->resno, oid_type, type_modifier,
						  0	 // varlevelsup
			);
	...
	List **hash_expr_types_out_list, CDXLTranslateContext *output_context)
{
	...
	for (ULONG ul = 0; ul < arity; ul++)
	{
		...
		const IMDType *md_type =
			m_md_accessor->RetrieveType(hash_expr_dxlop->MdidType());
		const IMDScalarOp *md_scalar_op = m_md_accessor->RetrieveScOp(
			md_type->GetMdidForCmpType(IMDType::EcmptEq));

		const IMDId *mdid_hash_type = md_scalar_op->GetLeftMdid();

		hash_expr_types_list = gpdb::LAppendOid(
			hash_expr_types_list, CMDIdGPDB::CastMdid(mdid_hash_type)->Oid());
...
CTranslatorDXLToPlStmt::TranslateSortCols(
	...
	Oid *sort_op_oids, bool *is_nulls_first)
{
	...
	BOOL is_resjunk)
{
	...
	Var *var =
		gpdb::MakeVar(OUTER, target_entry->resno, expr_oid, type_modifier,
					  0	 // varlevelsup
		);
	...
	const CDXLPhysicalCTAS *dxlop)
{
	...
	GpPolicy *distr_policy = (GpPolicy *) gpdb::GPDBAlloc(
		sizeof(GpPolicy) + num_of_distr_cols * sizeof(AttrNumber));
	...
	BitmapTableScan *bitmap_tbl_scan)
{
	...
	bitmap_idx_scan->logicalIndexInfo =
		gpdb::GetLogicalIndexInfo(rel_oid, index_oid);
	...
	TranslateIndexConditions(
		index_cond_list_dxlnode, table_descr, false /*is_index_only_scan*/,
		true /*is_bitmap_index_probe*/, index, md_rel, output_context,
		base_table_context, ctxt_translation_prev_siblings, &index_cond,
		&index_orig_cond, &index_strategy_list, &index_subtype_list);
