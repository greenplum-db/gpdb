...
// we have to define 'IVFFLAT_AM_OID' in src/include/catalog/pg_am_d.h
// done for other access methods.
// the "pg_am_d.h" file is GENERATED by src/backend/catalog/genbki.pl
#define IVFFLAT_AM_OID 17383
...
CTranslatorRelcacheToDXL::IsIndexSupported(Relation index_rel)
{
	...
	BOOL index_supported = gpdb::HeapAttIsNull(tup, Anum_pg_index_indexprs) &&
						   ...
						   (BTREE_AM_OID == index_rel->rd_rel->relam ||
							...
							IVFFLAT_AM_OID == index_rel->rd_rel->relam);
	if (index_supported)
	{
		return true;
	}
