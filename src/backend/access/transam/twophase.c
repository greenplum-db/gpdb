...
static void RecordTransactionCommitPrepared(TransactionId xid,
								...
								int nrels,
								RelFileNodeWithBackendAndStorageType *rels,
								int ninvalmsgs,
 ...
 *	3. RelFileNodeWithBackendAndStorageType[] (files to be deleted at commit)
 *	4. RelFileNodeWithBackendAndStorageType[] (files to be deleted at abort)
...
StartPrepare(GlobalTransaction gxact)
{
	...
	TransactionId *children;
	RelFileNodeWithBackendAndStorageType *commitrels;
	RelFileNodeWithBackendAndStorageType *abortrels;
	SharedInvalidationMessage *invalmsgs;
	...
	{
		save_state_data(commitrels, hdr.ncommitrels * sizeof(RelFileNodeWithBackendAndStorageType));
		pfree(commitrels);
	}
	if (hdr.nabortrels > 0)
	{
		save_state_data(abortrels, hdr.nabortrels * sizeof(RelFileNodeWithBackendAndStorageType));
		pfree(abortrels);
	}
	if (hdr.ninvalmsgs > 0)
	{
...
FinishPreparedTransaction(const char *gid, bool isCommit, bool raiseErrorIfNotFound)
{
	...
	RelFileNodeWithBackendAndStorageType *commitrels;
	RelFileNodeWithBackendAndStorageType *abortrels;
	RelFileNodeWithBackendAndStorageType *delrels;
	int			ndelrels;
	SharedInvalidationMessage *invalmsgs;
	...
	commitrels = (RelFileNodeWithBackendAndStorageType *) bufptr;
	bufptr += MAXALIGN(hdr->ncommitrels * sizeof(RelFileNodeWithBackendAndStorageType));
	abortrels = (RelFileNodeWithBackendAndStorageType *) bufptr;
	bufptr += MAXALIGN(hdr->nabortrels * sizeof(RelFileNodeWithBackendAndStorageType));
								...
								bool initfileinval)
{
	...
	if (nrels > 0)
	{
		...
		rdata[1].len = nrels * sizeof(RelFileNodeWithBackendAndStorageType);
