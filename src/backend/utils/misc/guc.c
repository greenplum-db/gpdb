...
static void write_auto_conf_file(int fd, const char *filename, ConfigVariable *head_p, bool emit_header);
 ...
 */
static void
write_auto_conf_file(int fd, const char *filename, ConfigVariable *head, bool emit_header)
{
	StringInfoData buf;
	ConfigVariable *item;
...
/*
 * Update last flushed XLOG location in config file.  This is used by WAL
 * receiver to update recovery.conf file with the last flushed XLOG segment
 * and TLI.
 */
void
UpdateLastFlushedXLog(char *conffilename, char *segnostr, char *tlistr,
					  ConfigVariable **head, ConfigVariable **tail)
{
	...
	replace_auto_config_value(head, tail, "last_flushed_seg", segnostr);
	replace_auto_config_value(head, tail, "last_flushed_tli", tlistr);
	
	/*
	 * Follow the same patter as in AlterSystemSetConfigFile: for crash
	 * safety, write to a temp file first and then rename it.
	 */
	Tmpfd = BasicOpenFile(TmpFileName,
	...
	PG_TRY();
	{
		...
		write_auto_conf_file(Tmpfd, conffilename, *head, false);
		 ...
		 * command.
		 */
		durable_rename(TmpFileName, conffilename, ERROR);
	}
	PG_CATCH();
	{
...
AlterSystemSetConfigFile(AlterSystemStmt *altersysstmt)
{
	...
	{
		/* Write and sync the new contents to the temporary file */
		write_auto_conf_file(Tmpfd, AutoConfTmpFileName, head, true);
