...
/* auto failover */
bool		gp_enable_check_coordinator = false;
int			gp_coordinator_id = UNINITIALIZED_GP_IDENTITY_VALUE;
int			gp_assigned_coordinator_id = UNINITIALIZED_GP_IDENTITY_VALUE;
...
struct config_bool ConfigureNamesBool_gp[] =
{
	...
	{
		{"gp_enable_check_coordinator", PGC_SIGHUP, GP_WORKER_IDENTITY,
			gettext_noop("This guc enables the QE to validate the coordinator ID to avoid"
						 " 2 active coordinator at the same time."),
			NULL,
		},
		&gp_enable_check_coordinator,
		false,
		NULL, NULL, NULL
	},
...
struct config_int ConfigureNamesInt_gp[] =
{
	...
	{
		{"gp_coordinator_id", PGC_BACKEND, GP_WORKER_IDENTITY,
			gettext_noop("The coordinator_id is to tracker who dispatches the query "
						 "for this QE. It's only valid for QE if gp_enable_check_coordinator "
						 "is true."),
			NULL,
			GUC_NOT_IN_SAMPLE | GUC_DISALLOW_IN_FILE
		},
	...
	{
		{"gp_assigned_coordinator_id", PGC_SIGHUP, GP_WORKER_IDENTITY,
			gettext_noop("The gp_assigned_coordinator_id is to record the ID of the coordinator."
						 "It's only checked if gp_enable_check_coordinator is true."),
			NULL,
		},
