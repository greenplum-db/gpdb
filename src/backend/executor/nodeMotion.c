...
ExecInitMotion(Motion *node, EState *estate, int eflags)
{
	...
	/* numHashSegments may be 0 if the plan is generated by ORCA */
	motionstate->numHashSegments = node->numHashSegments == 0 ?
									recvSlice->planNumSegments :
									node->numHashSegments;
	...
	if (motionstate->mstype == MOTIONSTATE_SEND && node->motionType == MOTIONTYPE_HASH)
	{
		...
		AssertImply(node->numHashSegments == 0, estate->es_plannedstmt->planGen == PLANGEN_OPTIMIZER);
