			...
			case HJ_SCAN_BUCKET:
				...
				if (joinqual == NULL || ExecQual(joinqual, econtext))
				{
					...
					if (parallel)
					{
						 ...
						 * for parallel joins, so we don't need to set match bit.
						 * Expermiments show that it's worth avoiding the shared
						 * memory traffic on large systems.
						 */
						Assert(!HJ_FILL_INNER(node));
					} else {
						/*
						 * This is really only need if HJ_FULL_INNER(node),
