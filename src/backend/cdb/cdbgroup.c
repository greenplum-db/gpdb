...
cost_2phase_aggregation(PlannerInfo *root, MppGroupContext *ctx, AggPlanInfo *info)
{
	...
	if (ctx->use_hashed_grouping)
	{
		...
		add_agg_cost(NULL, &input_dummy,
					 ...
					 ctx->numGroupCols,
					estimate_num_groups_per_segment(numGroups, input_rows, planner_segment_count(NULL)),
					 ctx->agg_costs);
	}
	else
	{
...
set_coplan_strategies(PlannerInfo *root, MppGroupContext *ctx, DqaInfo *dqaArg, Path *input)
{
	...
	group_num_persegment_1phase = estimate_num_groups_per_segment(darg_rows, input_rows * planner_segment_count(NULL),
														planner_segment_count(NULL));
