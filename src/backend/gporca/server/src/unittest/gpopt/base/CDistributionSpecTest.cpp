...
CDistributionSpecTest::EresUnittest_Hashed()
{
	...
	// Combine {A} true, {B} false: {B} false -> {A} true
	CDistributionSpecHashed *pdshashed6 = pdshashed2->Combine(mp, pdshashed5);

	GPOS_ASSERT(false == pdshashed6->FNullsColocated());
	GPOS_ASSERT(true == pdshashed6->PdshashedEquiv()->FNullsColocated());

	// Combine {B} true, {A} false: {A} false -> {B} true
	CDistributionSpecHashed *pdshashed7 = pdshashed4->Combine(mp, pdshashed3);

	GPOS_ASSERT(false == pdshashed7->FNullsColocated());
	GPOS_ASSERT(true == pdshashed7->PdshashedEquiv()->FNullsColocated());

	// Copy {A} false, {B} true
	CDistributionSpecHashed *pdshashed8 = pdshashed7->Copy(mp);

	GPOS_ASSERT(false == pdshashed8->FNullsColocated());
	GPOS_ASSERT(true == pdshashed8->PdshashedEquiv()->FNullsColocated());
