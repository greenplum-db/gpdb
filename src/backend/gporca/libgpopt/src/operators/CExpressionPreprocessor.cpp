...
// This is the method to check if the left outer join can be pruned.If prunable
 ...
 // expression.  The checks perfomed in this method are -
 // CheckJoinPruningCondnOnInnerRel method checks that the output columns and
 // the WHERE clause should only contain columns from the outer relation.
 // CheckJoinPruningCondnOnJoinCondn method finds the unique keys of the inner
 	...
 	CMemoryPool *mp, CExpression *pexprNew, CColRefSet *output_columns)
 {
 	...
 	// the join condition constains operator other than ScalarCmp(=)
 	// , BooleanAnd or contains a subquery then it will return a false.  If
 	// the aove checks are passed then it will populate the 'result' with
 	...
 	if (!CheckJoinPruningCondnOnJoinCondn(mp, pexprNew, result))
 	{
 		result->Release();
 		return pexprNew;
 	}
 ...
 bool
 CExpressionPreprocessor::CheckJoinPruningCondnOnJoinCondn(CMemoryPool *mp,
 														  CExpression *pexprNew,
 														  CColRefSet *result)
 {
 	GPOS_ASSERT(nullptr != pexprNew);
 	...
 	if (CPredicateUtils::FAnd(join_cond) &&
 		!CheckAndCondnInJoinCondn(mp, join_cond, inner_unique_keys, result,
 								  outer_rel_columns))
 	{
 		inner_unique_keys->Release();
 		return false;
 ...
 bool
 CExpressionPreprocessor::CheckAndCondnInJoinCondn(CMemoryPool *mp,
 ...
 void
 CExpressionPreprocessor::CheckUniqueKeyInJoinCond(CColRefSet *inner_columns,
 ...
 // any scalar subquery is present in the tree and find the ouput column of that
 												...
 												CExpression *pexprScalar)
 {
 	...
 	if (pexprScalar->DeriveHasSubquery())
 	{
 		...
 		else if (COperator::EopScalarSubqueryAny ==
 		...
 		JoinPruningTreeTraversal(mp, pexprScalar, pdrgpexpr,
 								 subquery_colrefset);
 ...
 // in 'CheckJoinPruningCondnOnInnerRel' method we can check that all the output
 // columns of the CLogicalLeftOuterJoin operator should be from the outer
 // relation.
 void
 CExpressionPreprocessor::ComputeOutputColumns(
 	CExpression *pexpr, CColRefSet *derived_output_columns,
 ...
 void
 CExpressionPreprocessor::JoinPruningTreeTraversal(
 ...
 // the outer relation.
 // 2. The unique keys of the inner relation should either be constant or equal to a
 // column from the outer relation.
 ...
 // foo and the keyset collection [p,r,s] of the inner table is present in the join condition
 // which is a constant or equal to a column of outer relation,the join can be pruned. When
 // the join is pruned the outer child of the CLogicalLeftOuterJoin is returned to the parent operator.
 //
 //	Algebrized query:
 //	+--CLogicalSelect
 //	|--CLogicalLeftOuterJoin
 										  ...
 										  CColRefSet *pcrsOutput)
 {
 	...
 	// If operator is not Left outer join, then
 	// return the current expression, no changes required.
 	output_columns->Release();
 	return pexprNew;
 }
